...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        train_count = 0
        train_dis_avg = 0.0
        if epoch == 5:  # Enable refinement after the first epoch
            opt.refine_start = True
        if opt.refine_start:
            estimator.eval()
            refiner.train()
        ...
        for rep in range(opt.repeat_epoch):
                    for i, data in enumerate(dataloader, 0):
                        ...
                        if opt.refine_start:
                            # Initial pose for SCM
                            initial_pose = torch.cat([pred_r, pred_t], dim=1)
                            
                            for ite in range(0, opt.iteration):
                                # Generate random view intervention (e.g., every other iteration)
                                interventions = {}
                                if ite % 2 == 0:
                                ...
                                losses = scm_criterion(
                                    ...
                                    new_points, features, idx, interventions
                                )
