...
color_palette = seaborn.color_palette('bright')
color_palette = [(int(e[2] * 255), int(e[1] * 255), int(e[0] * 255)) for e in color_palette]
...
def vis_pred(img_vis, cld, cam_fx, cam_cx, cam_fy, cam_cy, my_r, my_t, dist=np.zeros(5), color=(0, 255, 0)):
    cam_mat = np.matrix([[cam_fx, 0, cam_cx], [0, cam_fy, cam_cy], [0, 0, 1]])
    mat_r = quaternion_matrix(my_r)[0:3, 0:3]
    imgpts, jac = cv2.projectPoints(cld, mat_r, my_t, cam_mat, dist)
    num, _, dim = imgpts.shape
    for idx in range(num):
        img_vis = cv2.circle(img_vis, (int(imgpts[idx, :, 0]), int(imgpts[idx, :, 1])),
                             radius=2, thickness=2, color=color)
    return img_vis
...
for now in tqdm(range(0, 2949)):
    ...
    img = cv2.imread('{0}/{1}-color.png'.format(opt.dataset_root, testlist[now]), cv2.COLOR_BGR2RGB)
    img_vis = cv2.imread('{0}/{1}-color.png'.format(opt.dataset_root, testlist[now]), -1)
    # depth = np.array(Image.open('{0}/{1}-depth.png'.format(opt.dataset_root, testlist[now])))
    depth = cv2.imread('{0}/{1}-depth.png'.format(opt.dataset_root, testlist[now]), -1)
    ...
    for idx in range(len(lst)):
        ...
        try:
            ...
            img_vis = vis_pred(img_vis, cld[itemid], cam_fx, cam_cx, cam_fy, cam_cy, my_r, my_t,
                               dist=np.zeros(5), color=color_palette[idx % len(color_palette)])

        except ZeroDivisionError:
            print("PoseCNN Detector Lost {0} at No.{1} keyframe".format(itemid, now))

    cv2.imwrite(os.path.join(vis_path, '{}.png'.format(now)), img_vis)